...
. Press the 'i' key to activate the insert mode in the file. Copy paste the below yaml into the file and replace the *access token, service token, product id* values as shown in the image below and save it.
+
...
spec:
 ...
 config:
   ...
   system:
     name: system
     token: {replace with admin access token obtained from OpenShift Console UI}
     upstream:
       name: >-
   ...
   services:
     - id: {replace with the Product ID obtained from 3scale admin portal}
       token: {replace with Service Token obtained from 3scale admin portal}
       authorities:
         - '*'
       credentials:
         app_id:
